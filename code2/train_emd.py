...
class AE:
    def build_full_ae(self, trainable=True):
        ...
        self.points = self.Decoder_3D[-1].output[:, :, :, :3]
        logits = tf.squeeze(self.Decoder_3D[-1].output[:, :, :, 3:])
        self.part_label = tf.nn.softmax(logits, 2)
        self.part_label_pred = tf.argmax(self.part_label, 2)
        ...
        # emd matching
        self.match = tfappr.approx_match(tf.squeeze(self.input_3D.output), tf.squeeze(self.points))
        
        # VAE 1  Optimizer
        with tf.name_scope('Optimizer'):
            # 3D Optimizer    
            self.loss_3D = tf.reduce_mean(tfappr.match_cost(
                tf.squeeze(self.input_3D.output), tf.squeeze(self.points), self.match))
            
            self.loss_labels = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                logits=logits, labels=tf.squeeze(self.input_labels.output)))
            
            self.optimizer_3D = tf.train.AdamOptimizer(learning_rate=0.0001,
                                                       name='Adamvae3d').minimize(loss=self.loss_3D)
            
            self.optimizer_labels = tf.train.AdamOptimizer(learning_rate=0.0001,
                                                       beta1=self.beta,
    ...
    def __init__(self):
        ...
        self.fold1_features_conv = [53, 512, 512, 512, 512, 1024]
        ...
        for epoch in range(index, n_epochs):
            ...
            for i in range(num_train_file):
                ...
                for iteration in tqdm(range(num_batch)):
                    ...
                    l_3d, match, _ = self.session.run([self.loss_3D, self.match, self.optimizer_3D], 
                                                    feed_dict=feed_dict_vae)
                    match = np.argmax(match, 2)
                    loss_3d_arr.append(l_3d)
                    
                    target_labels = np.array([Batch_Domain_labels[i][match[i]] for i in range(len(match))])
            ...
            for iteration in tqdm(range(num_batch)):
                ...
                match = self.session.run(self.match, feed_dict={self.input_3D.output: Batch_Domain_3dT})
                match = np.argmax(match, 2)
                
                val_pair_arr.append(eval_pair(match))
                
                target_labelsT = [Batch_Domain_labelsT[i][match[i]] for i in range(len(match))]
