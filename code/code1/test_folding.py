...
class AE:
    def build_full_ae(self, trainable=True):
        ...
        for i in range(len(self.fold1_features_conv) - 1, 0, -1):
            self.Decoder_3D.append(utils.Conv3DLayer(last_layer=tt,
                                                     ...
                                                     strides=[1, 1, 1, 1],
                                                     activation=tf.math.tanh if i == 1 else tf.nn.relu,
        ...
        # self.part_label = self.Decoder_3D[-1].output[:, :, :, 3:]
    ...
    def __init__(self):
        ...
        self.fold1_features_conv = [3, 1024, 1024, 1024, 1024, 1024]
    ...
    def train_ae(self, n_epochs=2000):
...
#                 for iteration in range(num_batch):
                ...
                print("\r  %d %d-%d / 3d %f " % (epoch, iteration, i, np.mean(loss_3d_arr)))
            ...
            for iteration in tqdm(range(num_batch)):
                ...
                sample_3dT, sample_loss, summary_test = self.session.run(
                    [self.points, self.loss_3D, self.summary_groubs],
                    feed_dict={
            ...
            print(f'Epoch {epoch} Val 3d loss: {val_3d_loss}, pair: {val_pair}')
            ...
            summary_training = self.session.run(self.summary_groubs, feed_dict=feed_dict_vae)
