...
class AE:
    def build_full_ae(self, trainable=True):
        ...
        for i in range(len(self.fold1_features_conv) - 1, 0, -1):
            self.Decoder_3D.append(utils.Conv3DLayer(last_layer=self.Decoder_3D[-1],
                                                     ...
                                                     trainable=trainable))
            if i > 1:
                self.Decoder_3D.append(utils.MinusLayer(self.Decoder_3D[-1],
                                                        self.Decoder_3D[-1].output.shape.as_list(),
                                                        name='minus_layer_%d' % i,
                                                        trainable=True))
    ...
    def __init__(self):
        ...
        self.fold1_features_conv = [53, 512, 512, 512, 512]
        self.fold1_shape_conv = [1, 1, 1, 1, 1]
...
#                 for iteration in range(num_batch):
    ...
    def collect_result(self):
        ...
        ## load rotation data 
        test_data = np.load('test_data/test_data_30_0_0.npy')
        print('-----')
