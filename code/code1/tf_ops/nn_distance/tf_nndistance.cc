...
class NnDistanceOp : public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context)override{
            ...
            OP_REQUIRES(context,xyz1_tensor.dims()==3,errors::InvalidArgument("NnDistance requires xyz1 be of shape (batch,#points,#dims)"));
            ...
            int d=xyz1_tensor.shape().dim_size(2);
            OP_REQUIRES(context,xyz2_tensor.dims()==3,errors::InvalidArgument("NnDistance requires xyz2 be of shape (batch,#points,#dims)"));
            ...
            OP_REQUIRES(context,xyz2_tensor.shape().dim_size(2)==d,errors::InvalidArgument("NnDistance expects xyz1 and xyz2 have same dimension"));
...
class NnDistanceGradGpuOp : public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context)override{
            ...
            const float * grad_dist2=&grad_dist2_flat(0);
            Tensor * grad_xyz1_tensor=NULL;
            OP_REQUIRES_OK(context,context->allocate_output(0,TensorShape{b,n,d},&grad_xyz1_tensor));
            Tensor * grad_xyz2_tensor=NULL;
            OP_REQUIRES_OK(context,context->allocate_output(1,TensorShape{b,m,d},&grad_xyz2_tensor));
            auto grad_xyz1_flat=grad_xyz1_tensor->flat<float>();
            float * grad_xyz1=&grad_xyz1_flat(0);
            auto grad_xyz2_flat=grad_xyz2_tensor->flat<float>();
            float * grad_xyz2=&grad_xyz2_flat(0);
