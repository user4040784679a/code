...
def showpoints(points,c_gt=None, c_pred = None ,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(255,255,255),normalizecolor=True,ballradius=13):
    global showsz,mousex,mousey,zoom,changed, i
    ...
    while True:
        ...
        if cmd==ord('n'):
            ...
            cv2.imwrite('test%d/interpolation3d_%d.png' %(index, i),show)
        elif cmd==ord('f'):
            freezerot = ~freezerot
        if waittime!=0:
            break
...
if __name__=='__main__':
    ...
    if not os.path.exists('test%d' % index):
        os.makedirs('test%d' % index)

    # points = np.load('../Results_points_minus_dual/train_vln2dto3d%d.npy' % index)
    points = np.load('../Results_points_minus_dual_every/shape16_test_encoder3d%d.npy' % 1200)
    # points = np.load('../Results_points_minus_dual/train_3d.npy')
    # points = np.load('../from_95_to_27.npy')

    showpoints(points)
    # c_gt = np.zeros([2048, 3]) + 255
    # c_gt[0] = [0, 255, 255]
    # c_gt[100] = [255, 0, 255]
    # c_gt[200] = [255, 255, 0]
    # showpoints(points, c_gt)
