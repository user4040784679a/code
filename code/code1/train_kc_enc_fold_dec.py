...
def to_gather_idx(retx):
    arr = []
    
    for b in range(len(retx)):
        ret = retx[b]
        data = [[b, r] for r in ret]
        arr.append(data)
...
class AE:
    def build_full_ae(self, trainable=True):
        ...
        for i in range(1, len(self.t_features_conv)):
            self.T_net.append(utils.Conv3DLayer(last_layer=self.input_3D if i == 1 else self.T_net[-1],
                                                shape=[1, self.t_shape_conv[i],
                                                       self.t_features_conv[i - 1],
                                                       self.t_features_conv[i]],
                                                strides=[1, 1, 1, 1],
                                                name='T_Net_Layer%d_conv' % i,
                                                padding='VALID',
                                                trainable=trainable))

        self.T_net.append(utils.MaxpoolingLayer(last_layer=self.T_net[-1], num_point=self.points_num))

        for i in range(1, len(self.t_features_fc)):
            self.T_net.append(utils.Fully_Connected_up(last_layer=self.T_net[-1],
                                                       shape=[self.t_shape_fc[i],
                                                              self.t_shape_fc[i],
                                                              self.t_features_fc[i - 1],
                                                              self.t_features_fc[i]],
                                                       name='T_Net_Layer%d_fc' % i,
                                                       trainable=trainable))

        self.T_net.append(utils.TransformLayer(last_layer=self.input_3D,
                                               operator=self.T_net[-1],
                                               shape=[self.batch_size, 9],
                                               name='T_Net_layer_trans'))
        
        #  3D Main Encoder
        for i in range(1, len(self.features_3d)):
            self.Encoder_3D.append(utils.Conv3DLayer(last_layer=self.T_net[-1] if i == 1 else self.Encoder_3D[-1],
                                                     shape=[1,
                                                            self.shapes_3d[i],
        ...
        self.Encoder_3D.append(utils.Fully_Connected_up(last_layer=self.Encoder_3D[-1],
                                                        shape=[512],
                                                        name='Fc_up_3D_Layer1_conv'))
        self.Encoder_3D.append(utils.Fully_Connected_up(last_layer=self.Encoder_3D[-1],
                                                        shape=[512],
                                                        name='Fc_up_3D_Layer2_conv'))
        
        self.global_feature = self.Encoder_3D[-1].output
        
        # 3D Decoder
        self.Decoder_3D.append(utils.DuplicateLayer(tf.reshape(self.Encoder_3D[-1].output, [self.batch_size, 1, 1, -1]),
                                                    mutiple=2048, i=1))
        
        meshgrid = [[-0.3, 0.3, 32], [-0.3, 0.3, 64]]
        self.Decoder_3D.append(utils.GridLayer(self.Decoder_3D[-1], self.batch_size, meshgrid, name='grid'))
        
        for i in range(len(self.fold1_features_conv) - 1, 0, -1):
            self.Decoder_3D.append(utils.Conv3DLayer(last_layer=self.Decoder_3D[-1],
                                                     shape=[1,
        ...
        self.Decoder_3D.append(utils.ConcatLayer(self.Decoder_3D[-1], self.Decoder_3D[0]))
        
        for i in range(len(self.fold2_features_conv) - 1, 0, -1):
            self.Decoder_3D.append(utils.Conv3DLayer(last_layer=self.Decoder_3D[-1],
                                                     shape=[1,
                                                            self.fold2_shape_conv[i],
                                                            self.fold2_features_conv[i],
                                                            self.fold2_features_conv[i - 1]],
                                                     strides=[1, 1, 1, 1],
                                                     activation=None if i == 1 else tf.nn.relu,
                                                     normalizer=None,
                                                     name='Conv_fold2_3D_Layer%d_conv' % i,
                                                     padding='VALID',
                                                     trainable=trainable))
        ...
        with tf.name_scope('Optimizer'):
            ...
            self.optimizer_3D = tf.train.AdamOptimizer(learning_rate=0.0001,
                                                       name='point_optimizer').minimize(loss=self.loss_3D)
    ...
    def __init__(self):
        ...
        self.t_features_conv = [1, 64, 128, 1024]
        self.t_shape_conv = [0, 3, 1, 1]
        self.t_features_fc = [1024, 512, 256, 9]
        self.t_shape_fc = [1024, 512, 256, 9]
        ...
        self.fold1_features_conv = [3, 512, 512, 514]
        self.fold1_shape_conv = [1, 1, 1, 1]

        self.fold2_features_conv = [53, 512, 512, 515]
        self.fold2_shape_conv = [1, 1, 1, 1]
...
#                 for iteration in range(num_batch):
                for iteration in tqdm(range(num_batch)):
                    ...
                    l_3d, ret, _ = self.session.run([self.loss_3D, self.loss_idx, self.optimizer_3D], 
                                                    feed_dict=feed_dict_vae)
                    retb, retd = ret
                    loss_3d_arr.append(l_3d)
                    
                    target_labels = [Batch_Domain_labels[i][retd[i]] for i in range(len(retd))]
            ...
            for iteration in tqdm(range(num_batch)):
                ...
                target_labelsT = [Batch_Domain_labelsT[i][retd[i]] for i in range(len(retd))]
                
                sample_3dT, sample_loss, summary_test, test_acc = self.session.run(
                    [self.points, self.loss_3D, self.summary_groubs, self.accuracy],
                    feed_dict={
    ...
    def collect_global_feature(self):
        ...
        self.load_model('./Result_0806/train_folding_decoder/model/fine_tune.ckpt')
    ...
    def collect_result(self):
        ...
        for i in range(0, len(test_data), self.batch_size):
            ...
            sample_3dT, sample_labelsT, test_3d_loss, test_label_loss = self.session.run(
                [self.points, self.part_label_pred, self.loss_3D, self.loss_labels],
                feed_dict={
                    self.input_3D.output: Batch_Domain_3dT,
                    self.input_labels.output: target_labelsT})
        ...
        labelsT = np.concatenate(labelsT)
