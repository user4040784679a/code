...
if __name__=='__main__':
    ...
    with tf.Session('') as sess:
        point_num = 2048
        xyz1=np.random.randn(4, point_num, 3).astype('float32') + 5
        xyz2=np.random.randn(4, point_num, 3).astype('float32') + 10
        ...
        for i in range(1):
            s = time.time()
            b, d = sess.run([retb, retd])
            print(b)
            print(np.sum(b[0]) == sum(range(point_num)))
            print(np.sum(b[1]) == sum(range(point_num)))
            print(np.sum(b[2]) == sum(range(point_num)))
            print(np.sum(b[3]) == sum(range(point_num)))
#             print(time.time() - s)
