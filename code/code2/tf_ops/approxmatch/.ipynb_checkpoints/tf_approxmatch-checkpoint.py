...
if __name__=='__main__':
    ...
    pt_in=tf.placeholder(tf.float32,shape=(1,npoint,3))
    ...
    t_points = np.random.randn(1, 2048, 3).astype('float32')
    ...
    with tf.Session() as sess:
        ...
        for i in range(1001):
            trainloss,_=sess.run([loss,optimizer],feed_dict={pt_in: t_points})
            print(i, trainloss)
        
        print(trainloss)
        # trainmatch=sess.run(match,feed_dict={pt_in:tpoints})
